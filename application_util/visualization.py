...
class Visualization(object):
    ...
    def __init__(self, seq_info, update_ms, output_path=None):#eugeny added output path
        ...
        #eugeny - making video output
        self.output_path = output_path
        if self.output_path is not None:
            fourcc = cv2.VideoWriter_fourcc(*'mp4v')
            self.video_writer = cv2.VideoWriter(
                self.output_path, fourcc, 25, image_shape)
    ...
    def _update_fun(self, frame_callback):
        ...
        #Eugeny - trying to make video
        if self.output_path is not None:
            self.video_writer.write(self.viewer.image)
